...
class GsMixRenderPass : public IRenderPass, public IColorProvider, public IDepthProvider {
public:
    ...
    explicit GsMixRenderPass(
        const RenderGlobalResources& r,
        const std::string& scenePath);
    ~GsMixRenderPass() override;
...
private:
    ...
    void createPreprocessPipelineMix();
    void createMeshDepthTilePipeline();
    ...
    // dispatch
    void dispatchMeshDepthTile(VkCommandBuffer cmd, uint32_t imageIndex, uint32_t frameIndex); 
    void dispatchPreprocess(VkCommandBuffer cmd, uint32_t imageIndex, uint32_t frameIndex);
    ...
    void createMeshTileImages();
    void destroyMeshTileImages();
...
private:
    ...
    std::vector<std::shared_ptr<Image>> gsDepth;
    std::vector<VkImageLayout>          gsDepthLayouts;

    IGBufferProvider* occluder = nullptr;
    VkSampler meshDepthSampler = VK_NULL_HANDLE;

    std::vector<std::shared_ptr<Image>> meshTileDepth;
    std::vector<VkImageLayout>          meshTileDepthLayouts;
    VkExtent2D                          meshTileExtent{ 0, 0 };
    VkFormat                            meshTileDepthFormat = VK_FORMAT_R16G16B16A16_SFLOAT;
    ...
    std::shared_ptr<ComputePipeline> preprocessPipelineMix;
    std::shared_ptr<DescriptorSet> preprocessInputSet;
    std::shared_ptr<DescriptorSet> preprocessOutputSet;

    std::shared_ptr<DescriptorSet> preprocessMixDepthSet;

    // mesh tile depth pipeline
    std::shared_ptr<ComputePipeline> meshTilePipeline;
    std::shared_ptr<DescriptorSet>   meshTileInputSet;
    std::shared_ptr<DescriptorSet>   meshTileOutputSet;
