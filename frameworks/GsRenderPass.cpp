    ...
    camera(r.camera)
{
    ...
    if (!context)
        throw std::runtime_error("GsRenderPass: VulkanContext missing (RenderGlobalResources wrong)");
    if (!swapchain)
        throw std::runtime_error("GsRenderPass: Swapchain missing (RenderGlobalResources wrong)");
    if (!uniformBuffer)
        throw std::runtime_error("GsRenderPass: uniformBuffer missing (RenderGlobalResources wrong)");
    if (!camera)
        throw std::runtime_error("GsRenderPass: camera pointer missing (RenderGlobalResources wrong)");
    if (!scene)
        throw std::runtime_error("GsRenderPass: GSScene missing");
}
...
void GsRenderPass::onSwapchainResized()
{
    rebuildForResolutionChange();
}

void GsRenderPass::initialize()
{
    loadScene();
...
void GsRenderPass::update(float dt)
{
    ...
    static uint64_t c = 0;
    if ((c++ % 30) == 0) {
        spdlog::info("[GsRenderPass] totalSum(readback)={}", cachedNumInstances);
    }
}

void GsRenderPass::record(VkCommandBuffer cmd, uint32_t imageIndex, uint32_t frameIndex)
{
    ...
    dispatchPreprocess(cmd, frameIndex);
...
void GsRenderPass::createBuffers()
{
    const uint32_t N = static_cast<uint32_t>(scene->getNumVertices());
    gaussianCount = N;

    if (N == 0) {
        throw std::runtime_error("GsRenderPass::createBuffers: scene has 0 vertices");
    }
...
void GsRenderPass::createPreprocessPipeline()
{
...
void GsRenderPass::createPrefixSumPipeline()
{
...
void GsRenderPass::createRadixSortPipeline()
{
...
void GsRenderPass::createPreprocessSortPipeline()
{
...
void GsRenderPass::createTileBoundaryPipeline()
{
...
void GsRenderPass::createRenderPipeline()
{
...
void GsRenderPass::createRenderPipelineMix()
{
    ...
    if (!renderInputSet || !renderOutputSet) {
        throw std::runtime_error("GsRenderPass::createRenderPipelineMix: renderInputSet/renderOutputSet not ready");
    }
    ...
    if ((uint32_t)gbufs.size() < imageCount) {
        throw std::runtime_error("GsRenderPass::createRenderPipelineMix: occluder gbuffer image count mismatch");
    }
...
void GsRenderPass::createMeshDepthSampler()
{
    ...
    if (vkCreateSampler(context->device, &si, nullptr, &meshDepthSampler) != VK_SUCCESS) {
        throw std::runtime_error("GsRenderPass: failed to create meshDepthSampler");
    }
}

void GsRenderPass::destroyMeshDepthSampler()
{
...
void GsRenderPass::createGsOffscreenImages()
{
    destroyGsOffscreenImages();
    ...
    auto makeImage = [&](VkFormat fmt, VkExtent2D ex, uint32_t layers, VkImageViewType viewType) -> std::shared_ptr<Image> {
        ...
        if (vmaCreateImage(context->allocator, &img, &alloc, &image, &allocation, nullptr) != VK_SUCCESS)
            throw std::runtime_error("GsRenderPass: failed to create offscreen image");
        ...
        auto makeView = [&](VkImageViewType vt) -> VkImageView {
            ...
            if (vkCreateImageView(context->device, &view, nullptr, &iv) != VK_SUCCESS)
                throw std::runtime_error("GsRenderPass: failed to create offscreen image view");
            return iv;
            };
...
void GsRenderPass::destroyGsOffscreenImages()
{
...
void GsRenderPass::rebuildForResolutionChange()
{
    auto [width, height] = swapchain->swapchainExtent;
...
void GsRenderPass::ensureSortCapacity(uint32_t requiredInstances)
{
    const uint32_t N = static_cast<uint32_t>(scene->getNumVertices());
    if (N == 0) return;
    ...
    spdlog::warn("[GsRenderPass] Reallocating sort buffers. multiplier {} -> {} (requiredInstances={}, capacity={} -> {})",
...
void GsRenderPass::dispatchPreprocess(VkCommandBuffer cmd, uint32_t frameIndex)
{
    const uint32_t N = static_cast<uint32_t>(scene->getNumVertices());
    ...
    if (global.enableQueryManager && queryManager) {
        queryManager->writeTimestamp(
            frameIndex, cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, "preprocess_start");
    }
    ...
    if (global.enableQueryManager && queryManager) {
        queryManager->writeTimestamp(frameIndex, cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
            "preprocess_end");
    }
}

void GsRenderPass::dispatchPrefixSum(VkCommandBuffer cmd, uint32_t frameIndex)
{
    const uint32_t N = static_cast<uint32_t>(scene->getNumVertices());
    const uint32_t numGroups = (N + 255u) / 256u;
...
void GsRenderPass::dispatchPreprocessSort(VkCommandBuffer cmd, uint32_t frameIndex)
{
    const uint32_t N = static_cast<uint32_t>(scene->getNumVertices());
    const uint32_t numGroups = (N + 255u) / 256u;
...
void GsRenderPass::dispatchRadixSort(VkCommandBuffer cmd, uint32_t frameIndex)
{
    ...
    if (global.enableQueryManager && queryManager) {
        queryManager->writeTimestamp(frameIndex, cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, "sort_start");
    }
    ...
    if (global.enableQueryManager && queryManager) {
        queryManager->writeTimestamp(frameIndex, cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, "sort_end");
    }
}

void GsRenderPass::dispatchTileBoundary(VkCommandBuffer cmd, uint32_t frameIndex)
{
    const uint32_t numInstances = cachedNumInstances;
    if (numInstances == 0) {
    ...
    if (global.enableQueryManager && queryManager) {
        queryManager->writeTimestamp(frameIndex, cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, "tile_boundary_start");
    }
    ...
    if (global.enableQueryManager && queryManager) {
        queryManager->writeTimestamp(frameIndex, cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, "tile_boundary_end");
    }
}

void GsRenderPass::dispatchRender(VkCommandBuffer cmd, uint32_t imageIndex, uint32_t frameIndex)
{
    if (global.enableQueryManager && queryManager) {
        queryManager->writeTimestamp(frameIndex, cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, "3dgs_render_start");
    }
    ...
    if (useMix) {
        ...
        if (!camera) {
            throw std::runtime_error("GsRenderPass::dispatchRender: camera is null");
        }
    ...
    if (useMix) {
        if (gsDepth.empty() || !gsDepth[imageIndex]) {
            throw std::runtime_error("GsRenderPass::dispatchRender: useMix but gsDepth[imageIndex] is null");
        }
    ...
    if (global.enableQueryManager && queryManager) {
        queryManager->writeTimestamp(frameIndex, cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, "3dgs_render_end");
    }
}
